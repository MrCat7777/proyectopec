                ...
                <?php foreach ($pronosticos as $p): ?>
                    <div class="card" style="padding: 25px; margin-bottom: 0;">
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px; margin-bottom: 15px;">
                            <strong style="color: #38bdf8;"><?= htmlspecialchars($p['nombre']) ?></strong>
                        ...
                        <h3 style="text-align: center; margin-bottom: 15px; font-size: 1.5rem;">
                            <?= htmlspecialchars($p['local']) ?> <span style="color:#10b981;"><?= htmlspecialchars($p['goles_local']) ?> - <?= htmlspecialchars($p['goles_visitante']) ?></span> <?= htmlspecialchars($p['visitante']) ?>
                        </h3>
