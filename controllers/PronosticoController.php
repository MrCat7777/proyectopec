...
require_once __DIR__ . '/../libraries/PronosticoHelper.php';
...
class PronosticoController {
    ...
    public function buscarPartidos() {
        ...
        if ($busquedaActiva) {
            ...
            foreach ($calendarioPartidos as $partido) {
                $coincideSeleccion = true; $coincideFase = true;
                if (!empty($seleccion)) {
                    $busqueda = strtolower($seleccion); $local = strtolower($partido['local']); $visitante = strtolower($partido['visitante']);
                    if (strpos($local, $busqueda) === false && strpos($visitante, $busqueda) === false) $coincideSeleccion = false;
                }
                if (!empty($fase) && $fase !== 'todas') {
                    if (strpos($partido['fase'], $fase) === false) $coincideFase = false;
                }
                if ($coincideSeleccion && $coincideFase) $partidos_encontrados[] = $partido;
            }
        }
        require_once __DIR__ . '/../views/formulario_busqueda.php';
    }
    ...
    public function mostrarFormularioPronostico($errores = [], $datos = []) {
        ...
        require_once __DIR__ . '/../views/formulario_pronostico.php';
    }
    ...
    public function guardarPronostico($postData) {
        ...
        if ($local === $visitante && !empty($local)) $errores[] = "Las selecciones no pueden ser iguales.";
        if (!is_numeric($golesL) || $golesL < 0 || $golesL > 20) $errores[] = "Goles inválidos (0-20).";
        if (!is_numeric($golesV) || $golesV < 0 || $golesV > 20) $errores[] = "Goles inválidos (0-20).";
        ...
        $pronostico = [
            'nombre' => $nombre, 'correo' => $correo, 'local' => $local, 'visitante' => $visitante,
            'goles_local' => $golesL, 'goles_visitante' => $golesV, 'jugador' => $jugador,
            'comentario' => $comentario, 'analisis' => $analisis, 'fecha' => $fechaActual
        ];
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = 'reinaldojordan2006@gmail.com';
            $mail->Password   = 'iktx nwxc pbqc nwfx';
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port       = 587;
            $mail->CharSet    = 'UTF-8';
            $mail->SMTPOptions = array('ssl' => array('verify_peer' => false, 'verify_peer_name' => false, 'allow_self_signed' => true));

            $mail->setFrom('reinaldojordan2006@gmail.com', 'Sistema de Pronosticos');
            $mail->addAddress($correo, $nombre);
            $mail->addBCC('reinaldojordan2006@gmail.com');
            $mail->isHTML(true);
            $mail->Subject = 'Confirmacion de Pronostico - Mundial 2026';
            
            ob_start();
            include __DIR__ . '/../views/email_template.php';
        ...
        require_once __DIR__ . '/../views/resumen_pronostico.php';
    }

    public function listarPronosticos() {
        $modelo = new Pronostico();
        $pronosticos = $modelo->obtenerTodos();
        require_once __DIR__ . '/../views/listado_pronosticos.php';
    }
}
?>